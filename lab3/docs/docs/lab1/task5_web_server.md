...
- Отображение данных в HTML-формате
...
with open('grades.txt', 'a', encoding='utf-8') as f:
    f.write(f"{discipline}|{grade}|{timestamp}\n")
```
...
Сервер автоматически вычисляет:

- **Общее количество оценок** - сколько всего оценок добавлено
- **Средний балл** - среднее арифметическое всех оценок
- **Количество дисциплин** - уникальные дисциплины

```python
def calculate_statistics(grades):
    total = len(grades)
    avg = sum(int(g[1]) for g in grades) / total if total > 0 else 0
    disciplines = len(set(g[0] for g in grades))
    return total, avg, disciplines
```
...
- [RFC 7231: HTTP/1.1 Methods](https://tools.ietf.org/html/rfc7231#section-4)
- [Документация Python: urllib.parse](https://docs.python.org/3/library/urllib.parse.html)
