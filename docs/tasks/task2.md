...
sequenceDiagram
    ...
    C->>S: Подключение
    C->>S: JSON запрос с операцией и параметрами
    Note over S: Выполнение математических вычислений
    S->>C: Результат вычисления
    C->>S: Закрытие соединения
```

## Образовательная ценность

- **TCP протокол** - надежная доставка данных, установление соединения
- **JSON сериализация** - передача структурированных данных
- **Математические вычисления** - применение формул в программировании
- **Обработка ошибок** - валидация входных данных
- **Пользовательский интерфейс** - интерактивный ввод параметров
...
TCP протокол обеспечивает надежную доставку данных, что критично для математических вычислений. Использование JSON для сериализации данных делает систему гибкой и легко расширяемой.