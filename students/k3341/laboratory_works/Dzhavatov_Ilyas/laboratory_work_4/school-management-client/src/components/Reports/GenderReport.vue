      ...
      <div v-else-if="statistics.length > 0">
        ...
        <v-row class="mt-6">
          <v-col cols="12">
            <v-card variant="outlined">
              <v-toolbar color="grey-lighten-3" density="compact">
                <v-toolbar-title>Визуализация данных</v-toolbar-title>
              </v-toolbar>
              <v-card-text>
...
export default {
  ...
  methods: {
    ...
    createChart() {
      ...
      const classNames = this.statistics.map(item => item.class_name)
      const boysData = this.statistics.map(item => item.boys_count)
      const girlsData = this.statistics.map(item => item.girls_count)
      ...
      this.chartInstance = new Chart(ctx, {
        ...
        options: {
          ...
          plugins: {
            ...
            title: {
              display: true,
              text: 'Распределение учеников по полу в классах'
            }
          },
          scales: {
            y: {
