      ...
      <v-col cols="12">
        <v-card>
          <v-card-title class="headline">
            ...
            Доступные автомобили для аренды
          </v-card-title>
          ...
          <v-card-text v-else>
            ...
            <table class="elevation-1" style="width: 100%; border-collapse: collapse; background-color: white;">
              ...
              <tr style="background-color: #f5f5f5;">
                ...
                <th style="width: 18%; padding: 12px; border-bottom: 1px solid #e0e0e0; text-align: center;">Характеристики</th>
                <th style="width: 15%; padding: 12px; border-bottom: 1px solid #e0e0e0; text-align: center;">Действия</th>
              </tr>
              </thead>
              <tbody>
              <tr v-for="item in paginatedCars" :key="item.id" style="border-bottom: 1px solid #e0e0e0;">
                ...
                <td style="width: 10%; padding: 12px; text-align: center;">{{ item.current_mileage?.toLocaleString('ru-RU') || '0' }} км</td>
                ...
                <td style="width: 18%; padding: 12px; text-align: center;">
                  <div v-if="item.specification">
                    <div>{{ getEngineType(item.specification.engine_type) }}</div>
                    <small class="text-caption">
                      {{ getTransmissionText(item.specification.transmission) }} •
                      {{ getBodyTypeText(item.specification.body_type) }}
                    </small>
                    <div v-if="item.specification.engine_volume" class="text-caption">
                      {{ item.specification.engine_volume }} л • {{ item.specification.horsepower }} л.с.
                    </div>
                  </div>
                ...
                <td style="width: 15%; padding: 12px; text-align: center;">
                  ...
                  <v-btn
                      color="success"
                      small
                      @click="$router.push(`/cars/${item.id}/application`)"
              ...
              <tr v-if="paginatedCars.length === 0">
                <td colspan="9" style="padding: 16px; text-align: center; color: #9E9E9E;">
                  Нет доступных автомобилей для аренды
                </td>
              </tr>
              </tbody>
            </table>
