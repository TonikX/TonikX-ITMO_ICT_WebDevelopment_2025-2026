  ...
  <div class="variant-task">
    ...
    <div class="task-section">
      ...
      <div v-if="task1.result" class="result-card success">
        <h3>Результат: </h3>
        <p><strong>Маршрут: </strong> {{ task1.selectedRoute?.departure_point }} → {{ task1.selectedRoute?.destination_point }}</p>
    ...
    <div class="task-section">
      <h2>2. Маршруты с заполненностью менее указанного процента</h2>
      ...
      <div v-if="task2.result && task2.result.length > 0" class="result-card success">
        ...
        <div v-for="route in task2.result" :key="route. route_id" class="route-item">
          ...
          <p>Средняя заполненность: {{ route.average_occupancy }}%</p>
        </div>
      </div>
...
export default {
  ...
  computed: {
    // Задание 1
    'task1.selectedRoute'() {
      return this.routes.find(route => route.id === this.task1.selectedRouteId);
    },

    // Задание 3
    'task3.selectedFlight'() {
      return this.flights.find(flight => flight.id === this.task3.selectedFlightId);
    },

    // Задание 5
    'task5.selectedCompany'() {
      return this.companies.find(company => company.id === this.task5.selectedCompanyId);
    },
  },
  async created() {
    try {
  ...
  methods:  {
    ...
    async fetchRoutesBelowCapacity() {
      ...
      try {
        ...
        this.task2.result = response.data;
      } catch (err) {
        console.error('Ошибка получения данных:', err);
