              ...
              >
                ...
                <v-list-item-subtitle>
                  <span class="text-grey">
                    {{ teacher.classroom_number ? `Кабинет ${teacher.classroom_number}` : 'Без кабинета' }}
                  </span>
                </v-list-item-subtitle>
              </v-list-item>
            </v-list>
  ...
  methods: {
    ...
    async loadDashboardData() {
      ...
      try {
        ...
        const [teachersRes, studentsRes, schedulesRes, classesRes, classroomsRes] = await Promise.all([
          api.getTeachers({ limit: 5 }),
          studentApi.getStudents({ limit: 1 }),
          scheduleApi.getSchedules({ limit: 5 }),
          classesApi.getClasses({ limit: 1 }),
          classroomsApi.getClassrooms({ limit: 1 })
        ])
        ...
        this.stats[2].value = classesRes.data.count || classesRes.data.length || 0
        this.stats[3].value = classroomsRes.data.count || classroomsRes.data.length || 0
        ...
        // Ближайшие уроки
        const lessonsRaw = Array.isArray(schedulesRes.data)
          ? schedulesRes.data.slice(0, 5)
          : schedulesRes.data.results?.slice(0, 5) || []
        this.upcomingLessons = lessonsRaw.map(lesson => ({
          ...lesson,
          class_name: lesson.school_class_name || lesson.class_name,
