  ...
  methods: {
    ...
    async findLesson() {
      ...
      try {
        ...
        const lessons = Array.isArray(response.data) ? response.data : [response.data]
        this.foundLessons = lessons.map(lesson => ({
          ...lesson,
          class_name: lesson.school_class_name || lesson.class_name
        }))
        this.notify('success', 'Урок найден')
      } catch (error) {
        if (error.response?.status === 404) {
          this.foundLessons = []
          this.notify('info', 'В это время уроков нет')
        } else {
