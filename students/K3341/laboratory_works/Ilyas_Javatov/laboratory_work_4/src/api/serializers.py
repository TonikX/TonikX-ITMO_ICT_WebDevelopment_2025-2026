...
class ScheduleSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if teacher and subject and school_class:
            teaches_subject = teacher.subjects.filter(id=subject.id).exists()
            if not teaches_subject:
                raise serializers.ValidationError(
                    "Нельзя поставить в расписание: учитель не ведет этот предмет."
                )
            class_has_assignments = TeachingAssignment.objects.filter(
                subject=subject,
                school_class=school_class,
            ).exists()
            if class_has_assignments:
