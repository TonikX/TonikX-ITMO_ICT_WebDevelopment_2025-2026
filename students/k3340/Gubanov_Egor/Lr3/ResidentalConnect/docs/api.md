...
Все API endpoints требуют аутентификации через токен (кроме регистрации и тестового endpoint). Добавьте заголовок `Authorization: Token <your_token>` ко всем запросам.
...
GET /api/test-cors/
```
**Права доступа:** Публичный (без аутентификации)

**Описание:** Тестовый endpoint для проверки настройки CORS. Используется для отладки кросс-доменных запросов.
...
**Пример использования из браузера:**
```javascript
...
Подробнее о настройке CORS см. раздел [Настройка CORS](cors_setup.md).
...
```
GET /api/meter-readings/statistics/
```
**Права доступа:** Все аутентифицированные
- Жилец видит статистику только по своим квартирам
- Диспетчер видит общую статистику
...
```json
{
  ...
  "consumption_by_type": [
    {
      "meter_type": "cold_water",
      "meter_type_display": "Холодная вода",
      "total_consumption": 450.25,
      "count": 50,
      "avg_consumption": 9.01
    },
    {
      "meter_type": "hot_water",
      "meter_type_display": "Горячая вода",
      "total_consumption": 380.50,
      "count": 45,
      "avg_consumption": 8.46
    }
  ],
  "avg_values_by_type": [
    {
  ...
  "top_apartments_by_consumption": [
    {
      "apartment__number": "15",
      "apartment__building__address": "ул. Ленина, д. 10",
      "total_consumption": 125.50,
      "reading_count": 4
    }
  ]
}
...
## Агрегационные запросы (Statistics)
...
```
**Права доступа:** Все аутентифицированные
- Жилец видит статистику только по своим заявкам
- Мастер видит статистику по назначенным ему заявкам
- Диспетчер видит общую статистику
...
```json
{
  ...
  "category_distribution": [
    {
      "category__name": "Сантехника",
      "count": 60
    },
    {
      "category__name": "Электрика",
      "count": 40
    }
  ],
...
```json
{
  ...
  "occupied_apartments": 95,
  "vacant_apartments": 25,
  "occupancy_rate": 79.17,
  "total_area": 5400.50,
  "avg_apartments_per_building": 24.0,
  "avg_area_per_apartment": 45.00,
