<template>
  <v-container>
    <v-row>
      <v-col cols="12">
        <v-card>
          <v-card-title>Назначенные мне заявки</v-card-title>
          <v-card-text>
            <v-data-table
              :headers="headers"
              :items="requests"
            ...
            >
              ...
              <template v-slot:no-data>
                <div class="text-center py-8">
                  ...
                  <div class="text-h6 mt-4 text-grey">Нет назначенных заявок</div>
                </div>
              </template>
            </v-data-table>
          </v-card-text>
  ...
  methods: {
    async loadRequests() {
      ...
      try {
        ...
        const data = await serviceRequestsService.getAssignedToMe(params)
