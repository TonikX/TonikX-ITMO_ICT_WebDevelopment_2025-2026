# Task 5 — Реализация HTTP-сервера для учёта оценок

## Цель задания
Разработать простой HTTP-сервер на Python с использованием сокетов, который позволяет:
- просматривать журнал оценок,
- добавлять новые оценки через HTML-форму.
