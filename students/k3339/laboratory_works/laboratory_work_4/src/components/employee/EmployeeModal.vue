...
const props = defineProps({
  ...
  employeeData: {
    type: Object,
    default: () => ({first_name: "", second: "", patronymic: ""}),
  },
  clubs: {
    type: Array
  },
...
<template>
  <v-dialog :model-value="modelValue" @update:model-value="closeModal" persistent max-width="500">
    <v-card>
      <v-card-title>{{ mode === "add" ? "Добавить сотрудника" : "Редактировать сотрудника" }}</v-card-title>
      <v-card-text>
        <v-form @submit.prevent="handleSubmit">
          <v-text-field v-model="formData.passport" label="Паспортные данные" required></v-text-field>
