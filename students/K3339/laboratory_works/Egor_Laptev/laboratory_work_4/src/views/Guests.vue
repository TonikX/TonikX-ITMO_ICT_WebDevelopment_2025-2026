<template>
  <v-container>
    ...
    <v-dialog v-model="dialog" max-width="500">
      <v-card>
        <v-card-title>{{ editingItem ? 'Редактировать' : 'Добавить' }} гостя</v-card-title>
        <v-card-text>
...
const headers = [
  { title: 'ID', key: 'id' },
  { title: 'Номер паспорта', key: 'passport_number' },
...
const saveItem = async () => {
  ...
  try {
    if (editingItem.value) {
      await hotelAPI.guests.update(editingItem.value.id, form.value)
    } else {
...
const deleteItem = async (item) => {
  if (!confirm(`Удалить гостя ${item.last_name} ${item.first_name}?`)) return
  try {
    await hotelAPI.guests.delete(item.id)
    await loadData()
  } catch (err) {
